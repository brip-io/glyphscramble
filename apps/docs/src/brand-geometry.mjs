/* The BRIP mark and wordmark: the one drawing, and the one lettering.
 *
 * THIS FILE IS A COPY, and that is the thing to know before editing it.
 * Its master is `site/src/brand-geometry.mjs` in the private BRIP monorepo.
 * This repository is public and shares no runtime code with that one, so
 * the paths are transcribed here rather than imported, and an identity
 * change has to be brought across by hand. Do not redraw either by eye.
 *
 * What this file does buy is that the copy is made once. Every consumer
 * reads it from here — `components/brip-lockup.tsx` in the chrome, and
 * `scripts/generate-brand-assets.mjs` for the favicon and the share card —
 * so the three surfaces cannot drift from each other even though nothing
 * here can detect them drifting from the master.
 *
 * A plain .mjs rather than TypeScript because the generator runs under bare
 * node and `scripts/` is outside the type graph; the file has to be
 * importable from both sides of that line. tsconfig.json names it
 * explicitly in `include` for the same reason.
 *
 * TWO PATHS, NOT ONE, AND THE ORDER MATTERS. The mark is a folded ribbon
 * reading as a B: two dark arms with the ribbon’s pale underside showing
 * where it turns. FOLD is the whole silhouette and BODY is the dark part of
 * it, so the mark draws FOLD then BODY and the pale tone is what is left
 * uncovered rather than a third shape to hold in register. Flattened to one
 * tone the B reads as a blob with a notch in it.
 *
 * The ink box is 29.54x40, the largest whose farthest point survives
 * Android’s maskable crop when centred on the 64 tile: the middle-80% safe
 * circle has radius 25.6 and the mark’s corner sits at 24.6.
 */

export const MARK_W = 29.54;
...
export const WORD_W = 473.9;
export const WORD_H = 100;
export const WORD =
...
/** The lockup’s own proportions, shared by the chrome and the generators so
    a rendered asset and the header carry the same object at different
    sizes. Multiples of the wordmark’s cap height. */
export const MARK_TO_CAP = 1.423;
export const GAP_TO_CAP = 0.537;
