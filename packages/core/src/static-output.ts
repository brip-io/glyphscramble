...
function staticLoader(timeoutMs: number): string {
  return `(()=>{const t=${timeoutMs};for(const e of document.querySelectorAll('[data-glyphscramble-font][data-glyphscramble-state="loading"]')){const s=e.nextElementSibling;const fail=()=>{e.hidden=true;e.dataset.glyphscrambleState='error';if(s instanceof HTMLElement&&s.hasAttribute('data-glyphscramble-status'))s.dataset.glyphscrambleStatus='error'};(async()=>{let timer;try{if(!(s instanceof HTMLElement)||!s.hasAttribute('data-glyphscramble-status'))throw 0;const family=e.dataset.glyphscrambleFamily;if(!family)throw 0;const rootStyle=getComputedStyle(e);if(!rootStyle.fontFamily.includes(family))throw 0;const query=rootStyle.font||rootStyle.fontSize+' '+rootStyle.fontFamily;let text='';for(const n of [e,...e.querySelectorAll('*')]){const value=[...n.childNodes].filter(c=>c.nodeType===3).map(c=>c.textContent||'').join('');if(!value)continue;if(!getComputedStyle(n).fontFamily.includes(family))throw 0;text+=value}if(!text)throw 0;const timeout=new Promise((_,reject)=>{timer=setTimeout(()=>reject(Error('font timeout')),t)});const loaded=await Promise.race([document.fonts.load(query,text),timeout]);if(!Array.isArray(loaded)||loaded.length===0||!document.fonts.check(query,text))throw 0;clearTimeout(timer);s.hidden=true;s.dataset.glyphscrambleStatus='ready';e.dataset.glyphscrambleState='ready';e.hidden=false}catch{clearTimeout(timer);fail()}})()}})();\n`;
}
